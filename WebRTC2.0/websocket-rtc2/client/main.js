...
pc = new RTCPeerConnection(); //初始化RTCPeerConnection
...
function getData () {
    ...
    oReq.open("GET", "http://192.168.4.33:8080/data");
    oReq.send();
}
getData()

function reconnect (obj) { // 重连
    const { sdp } = obj.offer
    const answerData = new RTCSessionDescription({ type: "offer", sdp });
    answer(answerData)
}